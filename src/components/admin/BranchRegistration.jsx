...
const BranchRegistration = ({ setIsBranchRegistrationModalOpen }) => {

  const [isSelectLabModalOpen, setIsSelectLabModalOpen] = useState(false);

  const handleSelectLabClick = () => {
    setIsSelectLabModalOpen(true);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 border overflow-y-scroll">
      <div className="p-6 max-w-2xl w-full mx-auto bg-white shadow-2xl bg rounded-lg">
        ...
        <form>
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            ...
            <div className="w-full mx-auto">
              <span className="block mb-2 text-sm font-medium text-gray-900">Select Lab</span>
              <div className="relative">
                <button
                  onClick={() => handleSelectLabClick()}
                  className="w-1/2 p-2 bg-green-500 text-white rounded hover:bg-green-600"> Select Lab</button>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-8 justify-center w-full sm:w-auto mt-6">
            ...
            <button className="border border-red-500 hover:bg-red-100 hover:cursor-pointer hover:text-red-700 text-red-500 px-4 py-2 font-bold rounded-md text-sm flex items-center justify-center w-48" onClick={() => setIsBranchRegistrationModalOpen(false)}
            >
              CANCEL
            </button>
          </div>
      ...
      {isSelectLabModalOpen && (
        <LabSelection setIsSelectLabModalOpen={setIsSelectLabModalOpen} />
      )}
    </div>
  );
};
